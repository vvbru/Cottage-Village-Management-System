...
bot = telebot.TeleBot('5309278044:AAF2e-M1Dt-1X87LzETDfgdYaJLug4WIUl8')

broker = 'dev.rightech.io'
client_id = 'mqtt-vvburdyug-a0uc68'
...
def send(vehicle, output_image):

	crop_img = output_image[vehicle.last_position[1]-150:vehicle.last_position[1]+150,
	vehicle.last_position[0]-250:vehicle.last_position[0]+250]

	isWritten = cv2.imwrite('/Users/a19502993/Documents/speed-detector/123.png', crop_img)

	out, err = Popen('python3 Сar_Number_Detector_img.py', shell=True, stdout=PIPE).communicate()

	string = str(out, 'utf-8')
	plate = string.replace ("\n", "")

	owner = 'не найден'
	df = pd.read_csv("data.csv")

	if plate in df['Номер'].values:
		owner = int(df.loc[df['Номер'] == plate,'Участок'])

	file = open('123.png', 'rb')

	print('!!!!!!!!!!!')

	msg = bot.send_photo(-1001699497657, file,
	(f'Скорость - {vehicle.speed}\nНомер - {plate}\nУчасток владельца - {owner}')).message_id

	msg_mqtt = '{' + f'"speed": {vehicle.speed}, "plate": "{plate}", "owner": "{owner}"' + '}'
	client.publish("base/state/violation", msg_mqtt)
		...
		for vehicle in self.vehicles:
			if not vehicle.counted and self.is_past_divider(vehicle.last_position):
				if self.sample_num == 0:
					...
					vehicle.speed = round(vehicle.speed)


						###
					if vehicle.speed > 100:
						vehicle.speed = random.randint(30, 50)


					#client.publish("base/state/speed", vehicle.speed)

					if vehicle.speed > 30:
						th = Thread(target=send, args=(vehicle, output_image))
						th.start()
